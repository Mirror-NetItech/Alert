...
document.addEventListener('DOMContentLoaded', function() {
   ...
   //Function to extract content size
    ...
    function displayData(data, timestamp, index) { //Removed deviceEntriesContainer because it is not used and it cause issues
    ...
    function displayGroupedData() {
        ...
        for (const device in groupedData) {
            ...
            deviceEntries.forEach((item, i) => {
                const dataDiv = displayData(item, item.timestamp, i);
                deviceDiv.querySelector('.delete-button').addEventListener('click', (event) => { //Add Event Listener and delete button
                    event.stopPropagation();
                    moveDataToRecycleBin(i); // Move data to the Recycle bin using the correct index.
                });
    ...
    function moveDataToRecycleBin(index) { //Delete Data from database
        const itemToRemove = storedData[index];
